	...
	int in_sys, int staff)
{
  ...
  while (tp) {
	...
	break;
      case '-':
	if (f->flags & MARKS) p->put_rule("0.002 in", "0.6 in" );
	p->moveh (l->space /2.0);
	if (f->flags & MARKS) p->put_rule("0.002 in", "0.5 in" );
	p->moveh (l->padding/2.0);
	if (f->flags & MARKS) p->put_rule("0.002 in", "0.4 in" );
	//	p->moveh (text_used / -2.0);
	if (f->flags & MARKS) p->put_rule("0.002 in", "0.3 in" );
	p->moveh (f->font_sizes[font]/12.0 *
		  f_a[font]->fnt->get_width ('-')/ -2.0); // was -2.0
	if (f->flags & MARKS) p->put_rule("0.002 in", "0.2 in" );
