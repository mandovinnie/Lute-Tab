...
nmidi::~nmidi(){
  ...
  //  printf ("maxval %d maxi %d secondval %d secondi %d\n",
  //	  maxval, maxi,secondval,secondi);
...
void 
nmidi::nmidi_head(unsigned char pulses)
{
  format = 0;
  ...
  nbuffer->PutByte(0); 
  if (format - 0)
    nbuffer->PutByte(1); /* how many tracks */
  else
    nbuffer->PutByte(2); /* how many tracks */
  nbuffer->PutByte(0); nbuffer->PutByte(pulses);  /* pulses per quarter note */
}

void nmidi::midi_tail() {
